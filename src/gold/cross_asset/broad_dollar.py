...
DECISION (this file, not previously made anywhere): renormalize these 13
weights to sum to 1.0 rather than using the raw ~68.6-of-100 values
directly. This pipeline only ever has return data for these 13
currencies — the other ~51 economies in BIS's true Broad EER basket
...
USD moves uniformly against every one of these 13 — renormalizing
...
direction. AUD, EUR, and GBP are conventionally quoted with USD as the
QUOTE currency (AUD_USD, EUR_USD, GBP_USD — a pair RISE means USD
WEAKENED) and are negated before weighting so every term shares one
convention: positive contribution == USD strengthened against that
currency. The other 10 (CAD, CHF, JPY — Layer 1 USD_CAD/USD_CHF/USD_JPY;
CNH, KRW, SGD, HKD, TWD, NOK, IDR — Layer 2 dollar_basket, all quoted
USD-as-base per their yfinance_symbol values USDCNH=X etc.) are used
as-is. Confirmed against the live instruments_taxonomy.yaml Layer 1
forex block and Layer 2 dollar_basket group this session, not assumed.
...
GATE_1_EXTRACTION_DATE = "2026-09-12"

# ── Gate 1 empirical extraction, 12 Sep 2026 (2020_2022 vintage) ───────────
_RAW_BIS_WEIGHTS_PCT: dict[str, float] = {
    ...
    "JPY": 5.897981, "KRW": 4.183282, "NOK": 0.168854, "SGD": 1.341145,
    "TWD": 3.216667,
}
_RAW_WEIGHT_SUM = sum(_RAW_BIS_WEIGHTS_PCT.values())  # 68.599597

# Currency -> (Silver symbol, negate_before_weighting).
# negate=True for the 3 USD-is-quote pairs (AUD_USD, EUR_USD, GBP_USD).
...
def load_fx_returns(run_date: date, lookback_days: int) -> Optional[pl.DataFrame]:
    ...
    13 BIS_WEIGHTS symbols — 6 Layer 1 forex majors + 7 Layer 2
